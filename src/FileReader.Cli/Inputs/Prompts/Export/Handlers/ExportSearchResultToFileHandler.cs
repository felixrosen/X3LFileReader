...
public class ExportSearchResultToFileHandler : BaseHandler<(SearchResult SearchResult, FileReaderResult FileReaderResult), Task>
{
    public ExportSearchResultToFileHandler(IConsoleWriter consoleWriter) : base(consoleWriter)
    { }
    ...
    public override async Task<Task> Handle((SearchResult SearchResult, FileReaderResult FileReaderResult) input)
    {
        ...
        var sb = ConstructExportResult.Construct(input.SearchResult.FileColumns, input.SearchResult.Result);

        var filePath = await WriteToFile.Write(input.FileReaderResult.FileInfo.FileInfo,
                                               $"{input.SearchResult.SearchTerm}_Search",
                                               sb);
